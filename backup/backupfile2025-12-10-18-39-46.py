...
def create_gui():
    ...
    root.geometry("450x850")  # 初始大小（增加以容纳钓鱼记录）
    root.minsize(450, 700)    # 最小尺寸
    ...
    def on_frame_configure(event):
        ...
        root.geometry(f"450x{new_height}")
    ...
    # 创建窗口内的frame
    canvas_frame = canvas.create_window((0, 0), window=main_frame, anchor="nw", width=430)
    ...
    fish_record_card.pack(fill=X, pady=(10, 0))
    ...
    fish_tree.heading("名称", text="名称")
    fish_tree.heading("品质", text="品质")
    fish_tree.heading("重量", text="重量")

    fish_tree.column("时间", width=70, anchor="center")
    fish_tree.column("名称", width=120, anchor="w")
    fish_tree.column("品质", width=60, anchor="center")
    fish_tree.column("重量", width=70, anchor="center")

    fish_tree.pack(fill=X, pady=(0, 5))

    # 添加滚动条
    tree_scroll = ttkb.Scrollbar(fish_record_card, orient="vertical", command=fish_tree.yview)
    ...
    def update_fish_display():
        ...
        for record in reversed(filtered[-100:]):  # 最多显示100条
            ...
            time_short = time_str[:5] if len(time_str) >= 5 else time_str  # 只显示时:分
